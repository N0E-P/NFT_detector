...
async def on_message(message):
    ...
    if message.content.startswith("$NFT"):
        ...
        await message.channel.send(
            "To initialize me, please write the command _$collection_, and paste the address of your _NFT collection_."
        )
        await message.channel.send(
    ...
    if message.content.startswith("$collection"):
        ...
        await message.channel.send("You have only 2 things to do:")
        await message.channel.send(
            '> Please create a role named "NFT member" to make NFT owners recognisable, and to let them access special channels.'
        )
        await message.channel.send(
            "> And go on http://localhost:3000/ to enter your collection address that you just enter here"
        )
